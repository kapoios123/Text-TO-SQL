...
if db_mode == "SQLite":
    ...
    # Προσθήκη accept_multiple_files=True
    excel_files = st.sidebar.file_uploader("Upload Files", type=["xlsx", "xls", "csv"], accept_multiple_files=True)
    if excel_files:
        # Δημιουργούμε ένα shared engine στη μνήμη για όλα τα αρχεία
        if 'excel_engine_mem' not in st.session_state:
            st.session_state.excel_engine_mem = create_engine('sqlite:///', echo=False)
        
        # Φόρτωση όλων των αρχείων και λήψη του schema
        db_schema = excel_engine.load_multiple_files(excel_files, st.session_state.excel_engine_mem)
        st.sidebar.success(f"✅ {len(excel_files)} Files Loaded!")
...
if question and db_schema:
    ...
    # Δυναμικό context ημερομηνίας
    now = datetime.now()
    prompt = f"""
    You are a professional SQL expert.
    Current date is {now.strftime('%Y-%m-%d')} (Day: {now.strftime('%A')}).
    ...
    2. If multiple tables are mentioned, use JOINs based on common columns.
    3. For 'last year', use year {now.year - 1}. For 'this year', use {now.year}.
    4. Return ONLY the raw SQL SELECT query without markdown blocks.
    ...
    try:
        ...
        if is_safe_query(sql):
            if db_mode == "Excel":
                # Χρήση του shared engine για τα πολλαπλά αρχεία
                result_df = excel_engine.query_db(st.session_state.excel_engine_mem, sql)
